...
import { addQuestion, reorderQuestions, setActiveQuestion } from "@/store/slices/formSlice";
...
export default function QuestionsSidebar({ className, onItemClick }: { className?: string, onItemClick?: () => void }) {
    ...
    return (
        <aside className={className || "w-72 bg-white border-r border-zinc-200/80 flex flex-col h-[calc(100vh-4rem)] select-none"}>
                    ...
                    <DndContext onDragEnd={handleDragEnd} >
                        ...
                        >
                            {questions.map((question: Question) => (
                                <SidebarItem key={question.id} question={question} onItemClick={onItemClick} />
                            ))}
                        </SortableContext>
                    </DndContext>
                ) : (
